...
/* Helping struct to store device and file descriptor in one object */
...
static int device__poll(struct sensors_poll_device_t *device,
	...
	while (read(dev->fd, &jsevent, sizeof(jsevent)) > 0) {

		ALOGV("gsensor event %ld - %ld - %ld", jsevent.type, jsevent.number, event.value);
...
static int open_input_device(void) {
	...
	*filename++ = '/';
	...
	while ((de = readdir(dir))) {
		...
		if (ioctl(f, JSIOCGNAME(sizeof(name) - 1), &name) < 1) {
			name[0] = '\0';
		}
